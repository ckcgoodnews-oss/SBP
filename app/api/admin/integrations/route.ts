...
import { createIntegrationConnectionSchema } from '@/lib/validation/reports-integrations';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return fail('Failed to load integrations.', 500, error.message);
    }
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createIntegrationConnectionSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();
    ...
    if (error) {
      return fail('Failed to save integrations.', 500, error.message);
    }
