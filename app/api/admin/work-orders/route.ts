...
import { createWorkOrderSchema } from '@/lib/validation/core-crud';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load work-orders.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createWorkOrderSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();

    const { data, error } = await supabase
      .from('work_orders')
      .upsert(parsed.data, { onConflict: 'tenant_id,work_order_number' })
      .select('*')
      .single();

    if (error) return fail('Failed to save work-orders.', 500, error.message);
