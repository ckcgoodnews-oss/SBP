...
import { createServiceLocationSchema } from '@/lib/validation/workflow-crud';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load service-locations.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createServiceLocationSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();
    ...
    if (error) return fail('Failed to save service-locations.', 500, error.message);
