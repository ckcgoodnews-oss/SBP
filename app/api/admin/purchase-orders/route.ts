...
import { createPurchaseOrderSchema } from '@/lib/validation/inventory-operations';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load purchase-orders.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await supabase
      .from('purchase_orders')
      .upsert(parsed.data, { onConflict: 'tenant_id,po_number' })
      .select('*')
      .single();

    if (error) return fail('Failed to save purchase-orders.', 500, error.message);
