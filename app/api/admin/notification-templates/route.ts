...
import { createNotificationTemplateSchema } from '@/lib/validation/reports-integrations';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return fail('Failed to load notification-templates.', 500, error.message);
    }
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createNotificationTemplateSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();
    ...
    if (error) {
      return fail('Failed to save notification-templates.', 500, error.message);
    }
