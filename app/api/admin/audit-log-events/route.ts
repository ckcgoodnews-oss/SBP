...
import { createAuditLogEventSchema } from '@/lib/validation/reports-integrations';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) {
      return fail('Failed to load audit-log-events.', 500, error.message);
    }
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createAuditLogEventSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();
    ...
    if (error) {
      return fail('Failed to save audit-log-events.', 500, error.message);
    }
