...
import { createVendorSchema } from '@/lib/validation/inventory-operations';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load vendors.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    const parsed = createVendorSchema.parse(payload);
    const supabase = createServerAdminSupabaseClient();
    ...
    if (error) return fail('Failed to save vendors.', 500, error.message);
