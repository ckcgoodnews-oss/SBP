...
import { createPaymentSchema } from '@/lib/validation/workflow-crud';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load payments.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    if (error) return fail('Failed to save payments.', 500, error.message);
