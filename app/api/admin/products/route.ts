...
import { createProductSchema } from '@/lib/validation/core-crud';
...
export async function GET(request: Request) {
  try {
    ...
    if (error) return fail('Failed to load products.', 500, error.message);
...
export async function POST(request: Request) {
  try {
    ...
    if (error) return fail('Failed to save products.', 500, error.message);
