-- PATCH_007_fix_policy_quoting_and_rerunnable_iam.sql
...
-- Drop tenant-compatible policies first so this patch is safely rerunnable.
do $$
begin
  if to_regclass('public.iam_role_templates') is not null then
    execute 'drop policy if exists "role templates readable" on public.iam_role_templates';
  end if;
  if to_regclass('public.iam_permissions') is not null then
    execute 'drop policy if exists "permissions readable" on public.iam_permissions';
  end if;
  if to_regclass('public.departments') is not null then
    execute 'drop policy if exists "departments read" on public.departments';
    execute 'drop policy if exists "departments manage" on public.departments';
  end if;
  if to_regclass('public.iam_roles') is not null then
  ...
  if to_regclass('public.iam_role_permissions') is not null then
  ...
  if to_regclass('public.tenant_user_role_assignments') is not null then
    execute 'drop policy if exists "user role assignments read" on public.tenant_user_role_assignments';
    execute 'drop policy if exists "user role assignments manage" on public.tenant_user_role_assignments';
  end if;
  if to_regclass('public.user_location_permissions') is not null then
    execute 'drop policy if exists "user location permissions read" on public.user_location_permissions';
    execute 'drop policy if exists "user location permissions manage" on public.user_location_permissions';
  end if;
  if to_regclass('public.iam_invitations') is not null then
    execute 'drop policy if exists "iam invitations read" on public.iam_invitations';
    execute 'drop policy if exists "iam invitations manage" on public.iam_invitations';
  end if;
  if to_regclass('public.iam_sessions') is not null then
    execute 'drop policy if exists "iam sessions read" on public.iam_sessions';
  end if;
